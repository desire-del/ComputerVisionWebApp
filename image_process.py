...
demo_images = ["images/demo" +
               str(i)+".jpg" for i in range(1, number_of_demo_image+1)]
demo_videos = ["videos/"+ch for ch in ["africanGirl.mp4",
                                       "blue.mp4", "free.mp4", "friends.mp4"]]
...
def html_markdown(label, color='red', text_style='center', hmodel=2):
    st.markdown(
        f"<h{str(hmodel)} style = 'text-align:{text_style}; color:{color}'>{label}<h{hmodel}/>", unsafe_allow_html=True)


html_markdown("Computer Vision With OpenCV and Mediapipe",
              hmodel=1, color='black')
...
if mode == modes[0]:
    ...
    left_column.markdown(
        f"<h2 style = 'text-align:center; color:red'>Original Image<h2/>", unsafe_allow_html=True)
    ...
    if process == "Gray":
        right_column.markdown(
            f"<h2 style = 'text-align:center; color:red'>Gray Image<h2/>", unsafe_allow_html=True)
        right_column.image(to_gray(image))
    # Redimentionner l'image
    if process == "Resize":
        right_column.markdown(
            f"<h2 style = 'text-align:center; color:red'>Resized Image<h2/>", unsafe_allow_html=True)
        st.sidebar.markdown("---")
    ...
    if process == "TrackColor":
        ...
        hue_max = st.sidebar.slider(
            "Hue Max", min_value=0, max_value=179, value=179)
        sat_min = st.sidebar.slider("Sat Min", min_value=0, max_value=255)
        sat_max = st.sidebar.slider(
            "Sat Max", min_value=0, max_value=255, value=255)
        val_min = st.sidebar.slider("Val Min", min_value=0, max_value=255)
        val_max = st.sidebar.slider(
            "Val Max", min_value=0, max_value=255, value=255)

        # Convert the image to HSV image
        imgHsv = to_hsv(image)

        mask = mask(imgHsv, hue_min, sat_min, val_min,
                    hue_max, sat_max, val_max)
        imgResult = color_mask(image, mask)
        right_column.markdown(
            f"<h2 style = 'text-align:center; color:red'>HSV Image<h2/>", unsafe_allow_html=True)
        right_column.image(imgHsv, channels=channel)
        left_column.markdown(
            f"<h2 style = 'text-align:center; color:red'>Mask<h2/>", unsafe_allow_html=True)
        left_column.image(mask)
        right_column.markdown(
            f"<h2 style = 'text-align:center; color:red'>Color Detected<h2/>", unsafe_allow_html=True)
    ...
    if process == "Filter":
        ...
        if filter == filters[0]:
            ...
            # Type blur
            if blur == blurs[0]:
                x = st.sidebar.slider("Blur kernel size",
                                      1, 499, value=5, step=2)
                right_column.markdown(
                    f"<h2 style = 'text-align:center; color:red'>Smooth Image<h2/>", unsafe_allow_html=True)
                right_column.image(blur_image(image, x), channels=channel)

            # Type Median Blur
            if blur == blurs[1]:
                x = st.sidebar.slider("Blur kernel size",
                                      1, 361, value=5, step=2)
                right_column.markdown(
                    f"<h2 style = 'text-align:center; color:red'>Smooth Image<h2/>", unsafe_allow_html=True)
                right_column.image(median_blur_image(
                    image, x), channels=channel)
            ...
            if blur == blurs[2]:
                ...
                right_column.markdown(
                    f"<h2 style = 'text-align:center; color:red'>Smooth Image<h2/>", unsafe_allow_html=True)
                right_column.image(gaussian_blur_image(
                    image, x, sigmaX, sigmaY), channels=channel)
        ...
        if filter == filters[1]:
            ...
            if morphology_type == morphology[0]:
                ...
                iterations = st.sidebar.slider(
                    "Iteration", min_value=1, max_value=20, value=1)
                image_erode = img_erosion(to_gray(image), iterations)
                right_column.markdown(
                    f"<h2 style = 'text-align:center; color:red'>Erode Image<h2/>", unsafe_allow_html=True)
            ...
            if morphology_type == morphology[1]:
                ...
                iterations = st.sidebar.slider(
                    "Iteration", min_value=1, max_value=20, value=1)
                image_dilate = img_dilate(
                    to_canny(image, threshold1, threshold2), iterations)
                right_column.markdown(
                    f"<h2 style = 'text-align:center; color:red'>Dilate Image<h2/>", unsafe_allow_html=True)
            ...
            if morphology_type == morphology[2]:
                ...
                gradient = img_gradient(
                    to_canny(image, threshold1, threshold2))
                right_column.markdown(
                    f"<h2 style = 'text-align:center; color:red'>Gradient Image<h2/>", unsafe_allow_html=True)
        ...
        if filter == filters[2]:
            ...
            right_column.markdown(
                f"<h2 style = 'text-align:center; color:red'>Edge Detector<h2/>", unsafe_allow_html=True)
            # print(edge.shape)
...
if mode == modes[1]:
    ...
    face = st.sidebar.selectbox(
        "Face", options=["Run Video", "Face Mesh", 'Face Detector'])
    ...
    draw_spec1 = mp_drawing.DrawingSpec(
        thickness=1, circle_radius=1, color=(0, 255, 0))
    draw_spec2 = mp_drawing.DrawingSpec(
        thickness=1, circle_radius=1, color=(255, 0, 0))
    ...
    if face == "Face Mesh" or face == "Face Detector":
        ...
        max_face = max_face_placeholder.number_input(
            "Maximun Face detected", 1, 5, 1)
        left, midle, right, last = st.columns(4)
        left.markdown(
            "<h4 style = 'text-align:center; color:blue'>Frame Per Second<h4/>", unsafe_allow_html=True)
        midle.markdown(
            "<h4 style = 'text-align:center; color:blue'>Face Detected<h4/>", unsafe_allow_html=True)
        right.markdown(
            "<h4 style = 'text-align:center; color:blue'>Video Width<h4/>", unsafe_allow_html=True)
        last.markdown(
            "<h4 style = 'text-align:center; color:blue'>Video Height<h4/>", unsafe_allow_html=True)
        ...
        # Face Mesh code
        face_mesh = mp_face_mesh.FaceMesh(max_num_faces=max_face)
        ...
        while cap.isOpened():
            ...
            if face == "Face Mesh":
                result_face_mesh = face_mesh.process(frame)
                ...
                if result_face_mesh.multi_face_landmarks:
                    ...
                    for facelms in result_face_mesh.multi_face_landmarks:
                        mp_drawing.draw_landmarks(
                            frame, facelms, mp_face_mesh.FACE_CONNECTIONS, draw_spec1, draw_spec2)
                        face_count += 1

                if record:
                    out.write(frame)
            if face == "Face Detector":
                result_face_detector = face_detection.process(frame)
                frame.flags.writeable = True
                frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)

                face_count = 0

                if result_face_detector.detections:
                    for detection in result_face_detector.detections:
            ...
            k4.markdown(
                f"<h1 style = 'text-align:center; color:red'>{height}<h1/>", unsafe_allow_html=True)
...
with st.expander("About Me"):
    ...
    st.write("""My Contact""")
    st.markdown("""<h3 style = 'color:blue'>+22879627154<h3/>""",
